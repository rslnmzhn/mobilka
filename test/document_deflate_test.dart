...
void main() {
  final invalid = throwsA(isA<DocumentException>().having(
    (error) => error.code, 'code', 'invalid_document_deflate'));
  final overflow = throwsA(isA<DocumentException>().having(
    (error) => error.code, 'code', 'document_expansion_limit'));

  test('archive encoder stored, fixed and dynamic fixtures validate', () {
    final fixtures = [
      (utf8.encode('stored bytes'), 0, 0),
      (utf8.encode('hello'), 6, 1),
      (utf8.encode('the quick brown fox jumps over the lazy dog. ' * 1000), 6, 2),
    ];
    for (final (plain, level, type) in fixtures) {
      final encoded = Deflate(plain, level: level).getBytes();
      expect((encoded.first >> 1) & 3, type,
  ...
  test('fixed empty block accepts arbitrary final pad bits but no extra bytes', () {
    validateDocumentDeflate(Uint8List.fromList([3, 0]), 0);
    validateDocumentDeflate(Uint8List.fromList([3, 0xfc]), 0);
    expect(() => validateDocumentDeflate(Uint8List.fromList([3, 0, 0]), 0), invalid);
    expect(() => validateDocumentDeflate(Uint8List.fromList([3, 0, 3, 0]), 0), invalid);
  });

  test('missing final block, EOB, stored complement and reserved type fail', () {
    for (final bytes in [
      <int>[], [2, 0], [3], [7],
      [0, 0, 0, 255, 255],
      [1, 1, 0, 255, 255, 65],
    ]) {
      expect(() => validateDocumentDeflate(Uint8List.fromList(bytes), 0), invalid);
    }
    expect(() => validateDocumentDeflate(
      Uint8List.fromList([1, 1, 0, 254, 255]), 1), invalid);
  });

  test('every truncated prefix of an encoded dynamic stream is rejected', () {
    final plain = utf8.encode('a repeated document paragraph with words. ' * 300);
    final encoded = Deflate(plain).getBytes();
    for (var end = 0; end < encoded.length; end++) {
      expect(() => validateDocumentDeflate(
        Uint8List.sublistView(encoded, 0, end), plain.length), invalid,
        reason: 'truncated at byte $end');
    }
  });

  test('invalid backward distances and reserved fixed symbols fail', () {
    final noHistory = _BitsWriter()..field(3, 3)..code(1, 7)..code(0, 5);
    final reservedLiteral = _BitsWriter()..field(3, 3)..code(198, 8);
    final reservedDistance = _BitsWriter()
      ..field(3, 3)..code(113, 8)..code(1, 7)..code(30, 5);
  ...
  test('oversubscribed and incomplete code-length trees are rejected', () {
    for (final lengths in [[1, 1, 1, 0], [2, 2, 0, 0]]) {
      final bits = _BitsWriter()..field(5, 3)..field(0, 5)
        ..field(0, 5)..field(0, 4);
    ...
    expect(() => validateDocumentDeflate(
      _literalDynamic(literal: true, literalWidth: 2), 1), invalid);
    expect(() => validateDocumentDeflate(
      _literalDynamic(literal: true, extraLiteral: true), 1), invalid);
  });
  ...
  test('stored, literals and matches enforce exact output count', () {
    ...
    expect(() => validateDocumentDeflate(_literalDynamic(literal: true), 0), overflow);
    final matches = _BitsWriter()..field(3, 3)..code(113, 8)
      ..code(1, 7)..code(0, 5)..code(0, 7);
...
Uint8List _literalDynamic({required bool literal, int literalWidth = 1,
    bool extraLiteral = false}) {
  const order = [16, 17, 18, 0, 8, 7, 9, 6, 10, 5, 11, 4, 12, 3, 13, 2, 14, 1, 15];
  final bits = _BitsWriter()..field(5, 3)..field(0, 5)
    ..field(0, 5)..field(14, 4);
  for (final symbol in order.take(18)) {
    bits.field(symbol == 0 || symbol == literalWidth ? 1 : 0, 3);
  }
  for (var symbol = 0; symbol < 258; symbol++) {
    bits.field(symbol == 256 || (literal && symbol == 65) ||
      (extraLiteral && symbol == 66) ? 1 : 0, 1);
  }
