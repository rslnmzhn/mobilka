...
- Optional `web_search` uses a user-hosted SearXNG JSON endpoint for discovery only. It is disabled by default, permits globally routable HTTP only after endpoint-bound acknowledgement, never authenticates over HTTP, shares the public-source wire budget, and requires HTTPS `read_public_source` before content claims.
