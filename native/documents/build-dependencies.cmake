...
foreach(dep ZLIB PNG JPEG LEPTONICA TESSERACT)
  ...
  if(NOT EXISTS "${${dep}_LIBRARY}" OR IS_SYMLINK "${${dep}_LIBRARY}")
    message(FATAL_ERROR "Selected ${dep} library is not a regular archive")
  endif()
endforeach()
file(WRITE "${DOCUMENTS_BUILD_ROOT}/selected-libraries.json" "{\n")
set(separator "")
foreach(dep ZLIB PNG JPEG LEPTONICA TESSERACT)
  file(TO_CMAKE_PATH "${${dep}_LIBRARY}" library_path)
  string(REPLACE "\"" "\\\"" library_path "${library_path}")
  file(APPEND "${DOCUMENTS_BUILD_ROOT}/selected-libraries.json"
    "${separator}  \"${dep}\": \"${library_path}\"")
  set(separator ",\n")
endforeach()
file(APPEND "${DOCUMENTS_BUILD_ROOT}/selected-libraries.json" "\n}\n")
